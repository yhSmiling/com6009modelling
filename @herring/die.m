...
%death function for class HERRING
%agt=herring object
%cn - current agent number
%klld=1 if agent dies, =0 otherwise

%herring die if their food level reaches zero or they are older than max_age
...
klld=0;
thold=PARAM.H_MINFOOD;      %threshold minimum food value for death to occur
cfood=agt.food;             %get current agent food level
age=agt.age;                %get current agent age

if cfood<=thold|age>PARAM.H_MAXAGE      %if food level < threshold and age > max age then agent dies
