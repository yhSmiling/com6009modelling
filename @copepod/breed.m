...
%breeding function for class COPEPOD
%agt=copepod object
...
flim=PARAM.C_FOODBRD;       %minimum food level required for breeding
tlim=PARAM.C_BRDFQ;         %minimum interval required for breeding
...
if cfood>=flim&last_breed>=tlim  %if food > threshold and age > interval, then create offspring
   new=copepod(0,cfood/2,pos,PARAM.C_SPD,0);   %new copepod agent
   agt.food=cfood/2;                          %divide food level between 2 agents
   agt.last_breed=0;
   agt.age=age+1;
   IT_STATS.div_c(N_IT+1)=IT_STATS.div_c(N_IT+1)+1;             %update statistics
else                            
